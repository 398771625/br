...
def get_args():
    ...
    parser.add_argument('--nclt_protocol', type=str, default='standard',
            ...
            help='NCLT evaluation protocol. First sequence is DB, remaining are queries.')
    parser.add_argument('--nclt_ref_seq', type=str, default='2012-02-04',
            help='Reference/database sequence used when --nclt_protocol custom.')
    parser.add_argument('--nclt_query_seqs', type=str, default='2012-06-15,2012-09-28,2012-11-16,2013-02-23',
            help='Comma-separated query sequences used when --nclt_protocol custom.')
    parser.add_argument('--print_nclt_protocol', action='store_true',
            help='Print NCLT DB/query split before evaluation.')


    opt = parser.parse_args()
    return opt
def get_nclt_eval_seq(opt):
    protocol_map = {
        'standard': ['2012-01-15', '2012-02-04', '2012-03-17', '2012-06-15', '2012-09-28', '2012-11-16', '2013-02-23'],
        'cross_season': ['2012-02-04', '2012-06-15', '2012-09-28', '2012-11-16', '2013-02-23'],
        'near_season': ['2012-02-04', '2012-01-15', '2012-03-17'],
    }

    if opt.nclt_protocol == 'custom':
        query_seqs = [seq.strip() for seq in opt.nclt_query_seqs.split(',') if seq.strip()]
        if len(query_seqs) == 0:
            raise ValueError('When --nclt_protocol custom, --nclt_query_seqs must contain at least one sequence.')
        eval_seq = [opt.nclt_ref_seq.strip()] + query_seqs
    else:
        eval_seq = protocol_map[opt.nclt_protocol]

    return eval_seq


def print_nclt_protocol(protocol_name, eval_seq):
    print(f"====> NCLT protocol: {protocol_name}")
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        for epoch in range(opt.nEpochs):
            ...
            eval_seq = get_nclt_eval_seq(opt)
            eval_datasets = []
            eval_global_descs = []
            for seq in eval_seq:   
        ...
        if opt.print_nclt_protocol:
            print_nclt_protocol(opt.nclt_protocol, eval_seq)
        ...
        for ii in range(len(eval_seq[1:])):
            print('DB %s -> Query %s: %0.2f' % (eval_seq[0], eval_seq[ii+1], recalls_nclt[ii]*100))
