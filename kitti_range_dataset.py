...
from range_utils import load_range_png, random_roll
...
class InferDataset(data.Dataset):
    def __init__(self, seq, dataset_path='./datasets/KITTI/', sample_inteval=1):
        ...
        imgs_p.sort()
        self.imgs_path = [dataset_path + seq + '/range_imgs/' + imgs_p[i] for i in range(0, len(imgs_p), sample_inteval)]
...
class TrainingDataset(data.Dataset):
    def __init__(self, dataset_path='./datasets/KITTI/', seq='00'):
        ...
        for qi in range(len(self.poses)):
            ...
            self.positives[-1] = self.positives[-1][1:]
    ...
    def __getitem__(self, index):
        ...
        query = random_roll(load_range_png(self.imgs_path[index]))
        positive = random_roll(load_range_png(join(self.imgs_path[pos_idx])))

        negatives = []

        for neg_i in neg_idx:
            negative = random_roll(load_range_png(self.imgs_path[neg_i]))
            negatives.append(torch.from_numpy(negative.astype(np.float32)))

        negatives = torch.stack(negatives, 0)

        return query.astype(np.float32), positive.astype(np.float32), negatives, index
