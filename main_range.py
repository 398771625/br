...
import kitti_range_dataset as kitti_dataset
import nclt_range_dataset as nclt_dataset
...
def train_epoch(epoch, model, train_set):
    ...
    for iteration, (query, positives, negatives, indices) in enumerate(training_data_loader):
        ...
        for i in range(len(global_descs_Q)):
            max_loss = torch.max(
                criterion(global_descs_Q[i], global_descs_P[i], global_descs_N[num_negs * i:num_negs * (i + 1)]))
            loss += max_loss
...
if __name__ == "__main__":
    ...
    from range_model import RangeREIN as REIN
    ...
    if opt.mode.lower() == 'train':
        ...
        for epoch in range(opt.nEpochs):
            ...
            recalls_nclt = nclt_dataset.evaluateResults(eval_global_descs,
                                                        eval_datasets)  # (q_descs, db_descs, q_dataset, db_dataset)
        ...
        eval_seq = ['08']  # you can add more sequences if needed

        for seq in eval_seq:
            test_set = kitti_dataset.InferDataset(seq=seq, sample_inteval=5 if seq == '08' else 1)
        ...
        eval_seq = ['2012-01-15', '2012-02-04', '2012-03-17', '2012-06-15',

                    '2012-09-28', '2012-11-16', '2013-02-23']
